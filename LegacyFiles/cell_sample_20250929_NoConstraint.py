...
- cell_train_20250929_NoConstraint.pyで学習したモデルを使用
"""
import argparse
...
def main():
    ...
    # Create diffusion only (same as in cell_train_20250929_NoConstraint.py)
    _, diffusion = create_model_and_diffusion(
        **args_to_dict(args, model_and_diffusion_defaults().keys())
    )

    # Load gene list from data - with same preprocessing as cell_train_20250929_NoConstraint.py
...
def create_argparser():
    defaults = dict(
        clip_denoised=False,
        num_samples=500,  # サンプル数を500に変更
        batch_size=100,    # バッチサイズを調整
        use_ddim=False,
        model_path="output/checkpoint/backbone_NoConstraint/pbmc68k_NoConstraint_20250929/model001500.pt",  # 2000ステップのモデルパス
        sample_dir="output/simulated_samples/pbmc68k_NoConstraint_20250929_300samples.npz",              # NoConstraint版の出力ファイル（500サンプル版）
