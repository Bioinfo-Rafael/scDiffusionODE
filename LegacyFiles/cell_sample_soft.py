...
- 前処理でfilter_genesとfilter_cellsを適用してcell_train_softと一致させる
"""
import argparse
...
def main():
    ...
    # Create diffusion only (same as in cell_train_soft.py)
    _, diffusion = create_model_and_diffusion(
        **args_to_dict(args, model_and_diffusion_defaults().keys())
    )

    # Load gene list from data - AFTER preprocessing to match filtered genes (same as cell_train_soft.py)
    ...
    # Create GeneODE instance with soft=True (same as cell_train_soft.py)
    ...
    # Create hybrid model (same as in cell_train_soft.py)
...
def create_argparser():
    defaults = dict(
        clip_denoised=False,
        num_samples=3000,  # デフォルトを3000に変更
        batch_size=100,    # バッチサイズを調整
        use_ddim=False,
        model_path="output/checkpoint/backbone/pbmc68k_soft/model010000.pt",  # soft版のモデルパス
        sample_dir="output/simulated_samples/pbmc68k_soft.npz",              # soft版の出力ファイル
