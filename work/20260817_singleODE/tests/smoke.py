...
def _overrides(experiment: str, data_path: Path, edge_path: Path) -> list[str]:
    ...
    if experiment.startswith("ts_soft_tau80_"):
        values["t_s"] = 10
    return [
...
def run_smoke(experiment: str, batch_id: str) -> dict:
    ...
    try:
        ...
        grid = [
            sys.executable, str(SCRIPTS / "analyze_corr_norm_grid.py"),
            "--run-dir", str(run_dir), "--step", "20",
            "--max-cells", "4", "--batch-size", "2", "--device", "cpu",
        ]
        if _run(grid, result_dir / "corr_norm_grid.log", env=os.environ.copy()) != 0:
            raise RuntimeError(
                f"20-step analysis failed; see {result_dir / 'corr_norm_grid.log'}"
            )
        grid_manifests = sorted(run_dir.glob("analysis/corr_norm_step20_*/diagnostic_manifest.json"))
        if not grid_manifests:
            raise FileNotFoundError("20-step diagnostic manifest was not created")
        grid_manifest = read_json(grid_manifests[-1])
        grid_dir = grid_manifests[-1].parent
        required = {
            "timestep_metrics.csv",
            "correlation_every_20_steps.png",
            "norm_every_20_steps.png",
            "weighted_norm_every_20_steps.png",
        }
        if grid_manifest.get("t_values") != [0, 19]:
            raise AssertionError(f"unexpected dynamic grid: {grid_manifest.get('t_values')}")
        if not all((grid_dir / name).is_file() for name in required):
            raise AssertionError("20-step analysis outputs are incomplete")
        report["checks"]["dynamic_20_step_analysis_csv_json_plots"] = True
        report.update({
            "status": "completed",
            "finished_at": now_iso(),
