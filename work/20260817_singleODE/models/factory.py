"""Model factory for the four isolated 2026-08-17 single-ODE experiments."""
...
MODEL_FAMILIES = ("linear_hybrid_single_ode", "ts_soft_tau80_hybrid_single_ode")
...
def _validate_policy(config: Mapping, family: str) -> None:
    if int(config.get("K", 1)) != 1:
        raise ValueError("all 20260817_singleODE experiments require K=1")
    if str(config.get("gate_mode", "none")).lower() != "none":
        raise ValueError("single ODE requires gate_mode='none'")
    if _bool(config.get("reverse_coef", False)):
        raise ValueError("all single ODE experiments require reverse_coef=false")
...
def build_ode_from_config(
    ...
    *,
    ...
    common = dict(
        ...
        gate_mode=str(config.get("gate_mode", "none")),
