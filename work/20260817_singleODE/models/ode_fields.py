...
class BaseDirectHillField(nn.Module):
    """Shared TrainLoop-compatible field containing exactly one ODE."""
    ...
    def __init__(
        ...
        *,
        ...
        if self.is_lincomb or self.num_components != 1:
            raise ValueError("the 20260817_singleODE experiments require one ODE")
        self.time_dim = int(time_dim)
        self.hidden = int(hidden)
        self.dropout = _finite_float("dropout", dropout)
        self.gate_mode = str(gate_mode).lower()
        if self.gate_mode not in {"none", "single"}:
            raise ValueError("single ODE requires gate_mode='none'")
        ...
        if self.sparse_lambda > 0 or self.entropy_lambda > 0:
            raise ValueError("single ODE has no gate regularization")
    ...
    def get_gate_values(self, x: torch.Tensor, t=None) -> Dict[str, torch.Tensor]:
        del t
        x = x.float()
        probabilities = x.new_ones((x.shape[0], 1))
        return {
            "logits": x.new_zeros((x.shape[0], 1)),
    ...
    def component_outputs(self, x: torch.Tensor, t=None) -> torch.Tensor:
        ...
        output = production - self.delta.view(1, self.d) * x
        return output[:, None, :]
    ...
    def forward(self, x: torch.Tensor, t=None) -> torch.Tensor:
        ...
        self._cached_gate_reg = None
        self._cached_gate_stats = None
        return self.component_outputs(x, t)[:, 0, :]
    ...
    def compute_W(self, x: torch.Tensor, t=None) -> torch.Tensor:
        del t
        matrix = self.signed_regulatory_magnitude()
        return matrix[None, :, :].expand(x.shape[0], -1, -1)
...
class CenteredSignedHillField(BaseDirectHillField):
    ...
    def __init__(self, d: int, *, alpha_init_std: Optional[float] = None, **kwargs):
        ...
        self.alpha = nn.Parameter(
            torch.randn(self.d, self.d) * self.alpha_init_std
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rho = nn.Parameter(
            torch.randn(self.d, self.d) * self.rho_init_std
        )
