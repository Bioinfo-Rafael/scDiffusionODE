...
MODEL_FAMILIES = (
    "linear_hybrid_single_ode",
    "ts_soft_tau80_hybrid_single_ode",
)
EXPERIMENT_ORDER = (
...
EXPERIMENT_TO_PARTS = {
    "linear_singleODE_centered_signed_hill": (
        "linear_hybrid_single_ode", "centered_signed_hill"
    ),
    "linear_singleODE_shifted_hill_rho": (
        "linear_hybrid_single_ode", "shifted_hill_rho"
    ),
    "ts_soft_tau80_singleODE_centered_signed_hill": (
        "ts_soft_tau80_hybrid_single_ode", "centered_signed_hill"
    ),
    "ts_soft_tau80_singleODE_shifted_hill_rho": (
        "ts_soft_tau80_hybrid_single_ode", "shifted_hill_rho"
    ),
}
...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        raise ConfigurationError("single ODE K must be the integer 1") from exc
    if components != 1:
        raise ConfigurationError(f"all single ODE conditions require K=1, got {components}")
    if str(config.get("gate_mode", "")).lower() != "none":
        raise ConfigurationError("all single ODE conditions require gate_mode=none")
