...
This module deliberately lives below ``work/20260817_singleODE``. It may
...
def semantic_parameters(model: Any, ode_type: str) -> tuple[dict[str, np.ndarray], np.ndarray | None]:
    ...
    if ode_type == "centered_signed_hill":
        values["A"] = _as_numpy("A", _get_tensor_attr(ode, ("A",)))
        values["alpha"] = _as_numpy("alpha", _get_tensor_attr(ode, ("alpha",)))
        values["theta"] = _as_numpy("theta", _get_tensor_attr(ode, ("theta",)))
        values["bias"] = _as_numpy("bias", _get_tensor_attr(ode, ("b", "bias")))
        values["delta"] = _as_numpy("delta", _get_tensor_attr(ode, ("delta",)))
    elif ode_type == "shifted_hill_rho":
        values["A"] = _as_numpy("A", _get_tensor_attr(ode, ("A",)))
        values["rho"] = _as_numpy("rho", _get_tensor_attr(ode, ("rho",)))
        values["theta"] = _as_numpy("theta", _get_tensor_attr(ode, ("theta",)))
        values["bias"] = _as_numpy("bias", _get_tensor_attr(ode, ("b", "bias")))
        values["delta"] = _as_numpy("delta", _get_tensor_attr(ode, ("delta",)))
    else:
    ...
    *,
    ...
    for index, (name, value) in enumerate(values.items()):
        ...
        if name in {"alpha", "rho"}:
            note = f"{name} carries edge direction and is the off-mask penalty target"
        created.append(
    ...
    matrix_name = "alpha" if ode_type == "centered_signed_hill" else "rho"
    penalty_note = f"off-mask penalty target: signed {matrix_name}"
