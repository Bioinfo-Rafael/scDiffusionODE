...
EXPERIMENT_TO_PARTS = {
    name: tuple(name.split("__", 1)) for name in EXPERIMENT_ORDER
}
...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    expected = f"{family}__{ode_type}"
    if experiment != expected:
        raise ConfigurationError(
            f"experiment identity mismatch: experiment={experiment!r}, expected {expected!r}"
        )

    is_lincomb = family != "standard_hybrid_single"
    if is_lincomb:
        try:
        ...
        if str(config.get("gate_mode", "")).lower() != "softmax":
            raise ConfigurationError("all 20260803 LinComb conditions require gate_mode=softmax")

    regime_mode = str(config.get("regime_gate_mode", "none"))
    if family == "ts_soft_tau80_hybrid_lincomb":
        if regime_mode.lower() == "none":
            raise ConfigurationError("TS_soft_tau80 requires an enabled regime_gate_mode")
        ...
        raise ConfigurationError(f"{family} must use the standard (non-TS) Hybrid schedule")
