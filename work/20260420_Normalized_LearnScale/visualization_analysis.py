...
def extract_weights(path):
    ...
    # SmallScaleNet parameters
    scale_keys = [k for k in state.keys() if "scale_net" in k and "weight" in k]
    scale_params = []
    for k in scale_keys:
        w = state[k].flatten().detach().cpu().numpy()
        scale_params.append(w)
    weights["ScaleNet.params"] = np.concatenate(scale_params) if len(scale_params) > 0 else np.array([])
...
def plot_parameter_distributions(model_paths, output_dir):
    ...
    for col_idx, (label, wdict) in enumerate(weights_dict.items()):
        ...
        scale_params = wdict.get("ScaleNet.params")
        ...
        g_vals = torch.nn.functional.softplus(gamma).flatten().detach().cpu().numpy() if gamma is not None else []
        plot_hist(g_vals, axes[3, col_idx], "Gamma", "C3")

        plot_hist(ml_params, axes[4, col_idx], "CellUnet", "C4")

        plot_hist(scale_params, axes[5, col_idx], "SmallScaleNet", "C5")

    row_labels = ["W(Known)", "W(Unknown)", "Bias", "Gamma", "CellUnet", "SmallScaleNet"]
    for row in range(6):
...
def plot_velocity_umap(adata_real, model_path, output_dir, edge_tsv_path):
    ...
    hybrid_model = ODE_ML_HybridLearnedScale(ode_model=ode, ml_model=ml_model, timesteps=1000).to(device)
