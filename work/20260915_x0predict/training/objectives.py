...
from ..common import finite, assert_start_x
...
def training_loss(model, diffusion, x0, t, weights, config, noise=None):
    ...
    if config["objective"] not in ("stage1", "start_x", "ot"):
        raise ValueError("unsupported START_X objective")
    if not torch.equal(weights, torch.ones_like(weights)):
        raise ValueError("canonical objectives require uniform unweighted timesteps")
    if noise is None:
