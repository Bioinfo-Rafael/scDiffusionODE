...
def extract_weights(path):
    """
    20260416に置き換え
    Extract weights handling both old and new model keys
    ...
    # old CellUnet params (backward compatibility)
    ...
    # new factorized params
    f_theta_keys = [k for k in state.keys() if "f_theta" in k and "weight" in k]
    f_phi_keys   = [k for k in state.keys() if "f_phi" in k and "weight" in k]

    f_theta_params = []
    for k in f_theta_keys:
        w = state[k].flatten().detach().cpu().numpy()
        f_theta_params.append(w)

    f_phi_params = []
    for k in f_phi_keys:
        w = state[k].flatten().detach().cpu().numpy()
        f_phi_params.append(w)

    weights["f_theta.params"] = (
        np.concatenate(f_theta_params) if len(f_theta_params) > 0 else np.array([])
    )
    weights["f_phi.params"] = (
        np.concatenate(f_phi_params) if len(f_phi_params) > 0 else np.array([])
    )
...
def plot_parameter_distributions(model_paths, output_dir):
    ...
    fig, axes = plt.subplots(6, n_cols, figsize=(4 * n_cols, 18))
    if n_cols == 1:
    ...
    for col_idx, (label, wdict) in enumerate(weights_dict.items()):
        ...
        f_theta_params = wdict.get("f_theta.params", np.array([]))
        f_phi_params = wdict.get("f_phi.params", np.array([]))
        ml_params = wdict.get("ML.params", np.array([]))  # backward compatibility
        ...
        # 5th row: f_theta
        if len(f_theta_params) > 0:
            plot_hist(f_theta_params, axes[4, col_idx], "f_theta", "C4")
        else:
            # backward compatibility
            plot_hist(ml_params, axes[4, col_idx], "CellUnet", "C4")

        # 6th row: f_phi
        plot_hist(f_phi_params, axes[5, col_idx], "f_phi", "C5")

    # Labels
    row_labels = ["W(Known)", "W(Unknown)", "Bias", "Gamma", "f_theta", "f_phi"]
    for row in range(6):
...
def plot_velocity_umap(adata_real, model_path, output_dir, edge_tsv_path):
    ...
    from ODE.ode_analysis20260416 import GeneODE, ODE_ML_Hybrid
