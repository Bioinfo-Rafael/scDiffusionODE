...
def annotate_trajectory(ax, maximum):
    ...
    if maximum > 1000:
        ax.axvspan(
            1000, maximum, color="#009e73", alpha=0.12, label="Post-ODE integration"
        )
...
def plot_numeric(source, output):
    ...
    for filename in ("true_noise_metrics", "cellunet_vs_ode_metrics", "norm_metrics"):
    ...
    if convergence.exists():
        ...
        for name, columns, ylabel in (
            ...
            ax.set(
                xlabel="Post-ODE integration step k (diffusion conditioning held at t=0)",
                ylabel=ylabel,
                title=name,
            )
