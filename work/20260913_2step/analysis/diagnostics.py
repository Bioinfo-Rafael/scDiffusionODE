...
def timestep_grids():
    ...
    full = sorted(set(range(0, 1000, 20)) | {999})
    low = list(range(51))
    return full, low
...
def diagnostics(model, diffusion, real, output, *, batch_size, seed, division_epsilon):
    ...
    for timestep in sorted(set(full) | set(low)):
        ...
        for start in range(0, len(real), batch_size):
            ...
            metrics = {("norm", "true_noise", "l2"): norm(noise)}
            for name, value in outputs.items():
                finite(name, value)
                metrics.update(
                    {
                        ("truth", name, key): val
                        for key, val in per_cell_metrics(value, noise).items()
                    }
                )
                metrics[("norm", name, "l2")] = norm(value)
                metrics[("norm", name + "_to_true_noise", "ratio")] = norm(
                    value
                ) / norm(noise).clamp_min(division_epsilon)
            if hasattr(model, "ode_model"):
                metrics.update(
                    {
    ...
    for filename, rows in (
        ("true_noise_metrics.csv", truth_rows),
