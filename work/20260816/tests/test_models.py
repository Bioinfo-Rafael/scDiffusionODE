#!/usr/bin/env python3
"""Focused tests for the four direct-Hill experiments."""
...
class DirectHillTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.mask[1, 0] = 1.0
        cls.temp = tempfile.TemporaryDirectory(prefix="direct_hill_unit_")
    ...
    def config(self, family: str, ode_type: str) -> dict:
        prefix = "linear" if family == "linear_hybrid_lincomb" else "ts_soft_tau80"
    ...
    def field(self, ode_type: str):
        return factory.build_ode_from_config(
            self.config("linear_hybrid_lincomb", ode_type),
            self.genes,
            "cpu",
            mask=self.mask,
        )
    ...
    def test_A_is_nonnegative_and_sign_is_not_duplicated(self):
        for ode_type, signed_name in (
            ...
            self.assertIs(field.penalty_parameter(), getattr(field, signed_name))
            self.assertTrue(hasattr(field, signed_name))
            self.assertFalse(hasattr(field, "W"))

    def test_off_mask_penalty_reuses_signed_edge_parameter_and_full_mean(self):
        for ode_type, signed_name in (
            ("centered_signed_hill", "alpha"),
            ("shifted_hill_rho", "rho"),
        ):
    ...
    def test_hybrid_weights_and_branch_identity(self):
        ...
        with mock.patch.object(factory, "Cell_Unet", TinyCellUnet):
            linear = factory.build_model_from_config(
                self.config("linear_hybrid_lincomb", "centered_signed_hill"),
                self.genes,
                20,
                "cpu",
            ).eval()
            tau80 = factory.build_model_from_config(
                self.config("ts_soft_tau80_hybrid_lincomb", "centered_signed_hill"),
                self.genes,
                20,
                "cpu",
            ).eval()
            ...
            self.assertTrue(
                torch.equal(tau80.ode_branch_weight(x, t), torch.sigmoid((10.0 - t) / 80.0))
            )
            self.assertEqual(linear.ode_model.num_components, 8)
            coefficients = linear.ode_model.effective_coefficients(x, t)
            self.assertTrue(torch.allclose(coefficients.sum(-1), torch.ones(len(x))))
    ...
    def test_linear_vs_ts_only_changes_hybrid_policy(self):
        with mock.patch.object(factory, "Cell_Unet", TinyCellUnet):
            for ode_type in ("centered_signed_hill", "shifted_hill_rho"):
                linear = factory.build_model_from_config(
                    self.config("linear_hybrid_lincomb", ode_type), self.genes, 20, "cpu"
                )
                tau80 = factory.build_model_from_config(
                    self.config("ts_soft_tau80_hybrid_lincomb", ode_type), self.genes, 20, "cpu"
                )
                linear_shapes = {
                ...
                tau80_shapes = {
                    name: tuple(parameter.shape)
                    for name, parameter in tau80.named_parameters()
                }
                self.assertEqual(linear_shapes, tau80_shapes)
    ...
    def test_config_order_and_inherited_training_settings(self):
        ...
        inherited = json.loads(
            (REPO_ROOT / "work/20260803_ODE_hill_exp/configs/base.json").read_text()
        )
        ode_specific = {
            "schema_version", "ts_cache_path", "reference_sources",
            "hill_K_init", "hill_V_init", "racipe_n", "racipe_input_scale",
            "racipe_K_init", "racipe_G_init", "racipe_target_chunk_size",
            "racipe_log_production_min", "racipe_log_production_max",
            "exp_preactivation_min", "exp_preactivation_max",
            "theta_init", "regulation_A_init", "alpha_init_std", "rho_init_std",
            "target_chunk_size",
        }
        common_keys = (set(inherited) & set(self.base)) - ode_specific
        for key in common_keys:
            self.assertEqual(self.base[key], inherited[key], key)
