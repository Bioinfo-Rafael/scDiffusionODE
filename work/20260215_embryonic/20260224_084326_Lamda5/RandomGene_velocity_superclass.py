...
X = np.random.randn(*adata.shape).astype(np.float32)で遺伝子ベクトルをランダムにした時の作図。実験３
...
# 1. 初期設定とパスの追加
# ==========================================
def setup_environment(dir_name="PCA_velocity"): # ← 引数を追加
    ...
    # ↓ 受け取った dir_name を使うように変更
    base_output_dir = os.path.join(os.getcwd(), f"{timestamp}_{dir_name}")
...
# 3. メイン処理 (元 main 関数)
# ==========================================
def run_analysis(model_path, dir_name): # ← 名前を変えて引数を追加
    base_output_dir = setup_environment(dir_name) # ← dir_name を渡す
    ...
    # ==========================================
    # ★ 変更箇所: ODEモデルを用いた Velocity の計算
    # ==========================================
    print("Generating random input X using np.random.randn...")
    # adata.shape は (n_obs, n_vars) なので、それと同じサイズの正規分布乱数を生成
    X = np.random.randn(*adata.shape).astype(np.float32)
...
def main():
    # 1回目の実行
    model_1 = "/home/suzuki/Projects/scDiffusion/work/20260215_embryonic/20260224_084326_Lamda5/20260224_084328_train/checkpoints/pbmc68k_soft_20251127_hvg1024/ema_0.9999_100000.pt"
    run_analysis(model_path=model_1, dir_name="PCA_velocity_100000")

    # 2回目の実行
    model_2 = "/home/suzuki/Projects/scDiffusion/work/20260215_embryonic/20260224_084326_Lamda5/20260224_084328_train/checkpoints/pbmc68k_soft_20251127_hvg1024/ema_0.9999_000000.pt"
    run_analysis(model_path=model_2, dir_name="PCA_velocity_000000")
